...
(``research/experiments/evaluation-plan.md`` §1).
...
def methylation_features(row: Dict[str, str]) -> Dict[str, float]:
    ...
    (``research/knowledge/claim-boundaries.md``). ``mean_distance_to_read_end`` is included so
...
def assert_no_evaluation_fields(names: Sequence[str]) -> None:
    ...
    if offenders:
        raise ValueError(
            ...
            + " — see research/knowledge/claim-boundaries.md §2"
        )
...
def permute_methylation(rows: Sequence[Dict[str, str]], rng) -> List[Dict[str, str]]:
    ...
    (``research/hypotheses/hypotheses.md`` H3 acceptance).
    """
    methylation_columns = [
        "cpg_positions",
