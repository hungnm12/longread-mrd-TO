...
`requirements.txt` lists only `pysam`, `numpy`, `matplotlib`, `PyYAML` — so the
...
choice already recorded in `research/knowledge/scope.md`. `[repo]`
...
| python-pptx | 1.0.2 | system — used by `scripts/build_research_deck.py` |
