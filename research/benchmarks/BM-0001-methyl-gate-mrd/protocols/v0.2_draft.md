...
## 0. Revision log — resolution of review_001

Design iteration 2. Every blocking issue in `reviews/review_001.md` (including its addendum)
...
### C2 — CpG-density control (resolves B001, adopts N002)
...
scorable set as B1. `cut_C2` = the PCT-th percentile of `score_C2` over the scorable reads,
by the same rule. An ALT read is kept iff scorable and `score_C2 > cut_C2`.

FIND-0021 measured per-read AUC rising with modification-call count (0.586 / 0.621 / 0.675
across CpG-count buckets). C2 tests whether B1 is in effect selecting CpG-dense reads. It has
the same aggressiveness (top 5% by the same percentile rule) and the same scorable set.
...
- level detection under the frozen replicate rule (a level detects only if all 3 replicates
  return `detected`)
- ALT-read retention rate (kept / scorable ALT) and unscorable-ALT fraction
...
**EXPLORATORY — NOT PRIMARY BENCHMARK** (adopts N003): the same four arms restricted to
COLO829's 241 chr1 candidates, reported separately, so the chr1-versus-genome-wide scope
difference between the individuals is visible as a number rather than argued in prose. This
arm decides nothing in §20.
...
S3 (NO FALSE DETECTION)
    Leave-one-out over all five TF0 replicates per individual: each blank scored against the
    mean and spread of the other four. B1 returns `detected` on none of the ten.
...
└── results/
    ...
    ├── error_analysis.tsv
    ├── blank_selfcheck.tsv          S3, leave-one-out over five blanks
...
- 28 BAM passes. One pass per sample yields B0, B1, C1 (all 200 draws) and C2 — the controls
  are computed from the same in-memory per-read scores, so there is no extra I/O.
...
```text
- [ ] 28 BAMs exist, indexed, readable
...
- [ ] 200 C1 draws run; median and 5th/95th percentiles recorded
- [ ] Seeds recorded
- [ ] Deviations recorded
```
