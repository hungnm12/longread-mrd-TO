...
frame, in which the consensus is likewise built from a chr1 read set. It costs 12 further BAM
passes (COLO829's 5 blanks and 9 dilutions, minus none) and is declared here rather than
decided at run time. It is reported separately and **decides nothing in §20**.
...
- **40 BAM passes**: 28 for the primary benchmark (one pass per sample yields B0, B1, C1 with
  all 200 draws, and C2 — the controls are computed from the same in-memory per-read scores,
  so there is no extra I/O), plus 12 for the exploratory COLO829 chr1 re-run of §19.
