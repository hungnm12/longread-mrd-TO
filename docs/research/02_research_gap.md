...
[`01_paper_patterns.md`](./01_paper_patterns.md)). That is not a literature review. Three
...
[`01_paper_patterns.md`](./01_paper_patterns.md#position-2--background-suppression):
...
| HCC1395 dilution is not a valid proxy for the setting of interest | External validity limited | Already bounded in [`05_claim_boundaries.md`](./05_claim_boundaries.md) |

## Next action on this document

Run a documented literature search and record the queries, databases, date, and results in
[`decision_log.md`](./decision_log.md). Until that entry exists, every downstream document
